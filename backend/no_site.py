...
class SimpleHTTPServer:
    ...
    def handle_request(self, client_socket):
        ...
        try:
            ...
            if path == '/health':
                ...
                response = self.serve_static_file(path)
            ...
            error_response += f"Access-Control-Allow-Methods: GET, POST, OPTIONS\r\n"
            error_response += f"Access-Control-Allow-Headers: Content-Type\r\n"
            error_response += f"\r\nServer Error: {str(e)}"
    ...
    def serve_static_file(self, path):
        ...
        file_path = os.path.join(self.project_root, path.lstrip('/'))
        ...
        try:
            ...
            response_header += f"Access-Control-Allow-Origin: *\r\n"
            response_header += f"Access-Control-Allow-Methods: GET, POST, OPTIONS\r\n"
            response_header += f"Access-Control-Allow-Headers: Content-Type\r\n"
            response_header += f"\r\n"
...
def main():
    """主函数"""
    print("=" * 60)
    print("Python在线编译器后端服务（无site模块版）")
    print("=" * 60)
